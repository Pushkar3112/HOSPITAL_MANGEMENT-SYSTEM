...
const PatientDoctorChat = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (sid) {
          const found = res.data.data.find((s) => s.id === sid);
  ...
  const sendMessage = () => {
    ...
    const otherId = user.role === "PATIENT" ? activeSession.doctorId : activeSession.patientId;
  ...
  const getOtherPerson = (session) => user.role === "PATIENT" ? session.doctor : session.patient;
  ...
  return (
    <AppLayout title="Messages" subtitle="Real-time chat with your healthcare team">
          ...
          <div style={{ flex: 1, overflowY: "auto" }}>
            {loadingSessions ? (
              ...
              <div className="empty-state" style={{ padding: "40px 20px" }}>
                ...
                <div className="empty-title" style={{ fontSize: 15 }}>No conversations</div>
                <div className="empty-subtitle" style={{ fontSize: 12 }}>Book an appointment to start chatting with a doctor</div>
              </div>
            ) : (
              sessions.map((session) => {
                ...
                return (
                  <div key={session.id} className={`chat-session-item ${isActive ? "active" : ""}`} onClick={() => selectSession(session)}>
                    <div className="user-avatar" style={{ width: 40, height: 40, fontSize: 16 }}>
                      {other?.avatar ? <img src={other.avatar} alt={other.name} /> : other?.name?.[0] || "?"}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div className="chat-session-name">{other?.name || "Unknown"}</div>
                      {other?.doctorProfile && (
                        <div style={{ fontSize: 11, color: "var(--accent-primary)", fontWeight: 600, marginBottom: 2 }}>
                          {other.doctorProfile.specialization}
                        </div>
                      )}
                      <div className="chat-session-last">{session.lastMessage || "Start a conversation"}</div>
                    </div>
        ...
        <div className="chat-main">
          {!activeSession ? (
            <div className="empty-state" style={{ flex: 1 }}>
              ...
              <div className="empty-subtitle">Choose a conversation from the left to start messaging</div>
            </div>
          ) : (
            <>
              {/* Header */}
              <div className="chat-header">
                <div className="user-avatar" style={{ width: 40, height: 40 }}>
                  {getOtherPerson(activeSession)?.avatar ? (
                    <img src={getOtherPerson(activeSession).avatar} alt="" />
                  ) : (
                    getOtherPerson(activeSession)?.name?.[0] || "?"
                  )}
                </div>
                <div>
                  <div style={{ fontWeight: 700, color: "var(--text-primary)", fontSize: 15 }}>
                    {getOtherPerson(activeSession)?.name || "Unknown"}
                  </div>
                  {user.role === "PATIENT" && activeSession.doctor?.doctorProfile && (
                    <div style={{ fontSize: 12, color: "var(--accent-primary)" }}>
                      {activeSession.doctor.doctorProfile.specialization}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="chat-messages">
                {loadingMessages ? (
                  ...
                  <div className="empty-state">
                    ...
                    <div className="empty-subtitle">Send a message to begin your conversation</div>
                  </div>
                ) : (
                  messages.map((msg) => {
                      ...
                      <div key={msg.id} className={`chat-message ${isSent ? "sent" : "received"}`}>
                        {!isSent && (
                          <div className="user-avatar" style={{ width: 32, height: 32, fontSize: 13, flexShrink: 0 }}>
                            {msg.sender?.avatar ? <img src={msg.sender.avatar} alt="" /> : msg.sender?.name?.[0]}
                          </div>
                        )}
                        <div>
              ...
              <div className="chat-input-area">
                ...
                />
                <button className="chat-send-btn" onClick={sendMessage} disabled={!input.trim()} style={{ opacity: !input.trim() ? 0.5 : 1 }}>
                  ➤
                </button>
              </div>
            </>
